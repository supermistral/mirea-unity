...
public class GravityDirection : MonoBehaviour
{
    ...
    public Vector3 GetDirection(Vector3 pos)
    {
        for (int i = 0; i < _corners.Count; i++)
        {
            ...
            Vector2 p1 = new(pos.x, pos.y);
            Vector2 p2 = new(transform.position.x, transform.position.y);
            Vector2 p3 = new(_corners[i].position.x, _corners[i].position.y);
            Vector2 p4 = new(_corners[next].position.x, _corners[next].position.y);

            if (LineUtil.IntersectLineSegments2D(p1, p2, p3, p4, out Vector2 intersection))
            {
                Vector2 newDirection = new(p4.x - p3.x, p4.y - p3.y);
                return new Vector3(-newDirection.y, newDirection.x, pos.z).normalized;
            }
        }
